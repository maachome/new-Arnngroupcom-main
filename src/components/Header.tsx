...
export function Header() {
  ...
  return (
    <header className={`fixed left-0 right-0 z-50 py-3 px-8 flex-shrink-0 transition-all duration-500 ${
        ...
        <nav className="hidden lg:flex items-center gap-8">
          <Link 
            ...
            className={`text-sm tracking-wide hover:opacity-80 transition-opacity ${
              location.pathname === '/' ? `border-b-2 ${isScrolled ? 'border-white' : 'border-gray-900'}` : ''
            }`}
          >
            HOME
          </Link>
          ...
          <Link 
            to="/contact" 
            className={`text-sm tracking-wide hover:opacity-80 transition-opacity ${
              location.pathname === '/contact' ? `border-b-2 ${isScrolled || location.pathname !== '/' ? 'border-white' : 'border-gray-900'}` : ''
            }`}
          >
            CONTACT
          </Link>
          ...
          <SheetContent side="right" className="w-full bg-[#2d3e5f] text-white border-l border-white/10">
            ...
            <nav className="flex flex-col gap-1 mt-8">
              ...
              <Link 
                ...
                onClick={() => setIsMobileMenuOpen(false)}
                className={`px-4 py-3 text-sm tracking-wide hover:bg-white/10 transition-colors rounded-lg ${
                  location.pathname === '/about-us' ? 'bg-white/10' : ''
                }`}
              >
                ABOUT US
              </Link>
